...
def deploy_async_endpoint(model_uri, image_uri=None):
    ...
    # BILLING: NOT pay-per-API-call. You pay for instance HOURS (~$0.736/hr) while the
    # endpoint exists. Delete the endpoint when idle to stop charges (see SAGEMAKER_AVOID_CHARGES.md).
    ...
    print(f"⚠️  Billing: by instance hour (not per API call). Delete endpoint when idle to avoid charges.")
...
def deploy_serverless_endpoint(model_uri, image_uri=None):
    """
    Deploy the model as a Serverless Inference endpoint (PAY-PER-USE)

    Pricing: Pay only for inference compute time (per millisecond)
    - No cost when idle
    - ~$0.000012 per second of inference time (memory: 4GB)
    - Automatic scaling from 0 to many instances

    Limitations:
    - No GPU support (CPU only - slower for Cellpose)
    - Max payload: 4MB
    - Max inference time: 60 seconds
    - Cold start latency when scaling from 0

    Args:
        model_uri: S3 URI of the model artifact
        image_uri: ECR URI of the Docker image
    """
    from sagemaker.serverless import ServerlessInferenceConfig

    print("Deploying Serverless Inference endpoint (PAY-PER-USE)...")
    print("⚠️  NOTE: Serverless = CPU only (no GPU). Cellpose will be slower.")

    # Clean up any existing endpoint/config first
    cleanup_existing_endpoint()

    if not image_uri:
        raise ValueError("image_uri is required for serverless deployment")

    # Configure serverless inference
    serverless_config = ServerlessInferenceConfig(
        memory_size_in_mb=6144,  # Options: 1024, 2048, 3072, 4096, 5120, 6144 MB
        max_concurrency=20,       # Max concurrent invocations (1-200)
    )
    ...
    model = Model(
        ...
        env={
            'SAGEMAKER_MODEL_SERVER_WORKERS': '1',
            'SAGEMAKER_MODEL_SERVER_TIMEOUT': '60'  # Max 60s for serverless
        }
    )

    print("\nDeploying serverless endpoint (CPU only)...")
    print(f"Memory: 6144 MB, Max concurrency: 20")
    print("💰 Pricing: ~$0.000012/second of inference time")
    print("💰 $0 when idle (no constant hourly cost)")
    ...
    try:
        ...
        print(f"\n✅ Serverless endpoint deployed: {ENDPOINT_NAME}")
        print("\n💡 Usage notes:")
        print("   - First request after idle may have cold start delay (10-60s)")
        print("   - Subsequent requests are fast")
        print("   - Scales automatically based on demand")
        print("   - You only pay for actual inference time")
        return predictor
    except Exception as e:
        error_msg = str(e)
        if "Failed" in error_msg and "health check" in error_msg.lower():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--serverless', action='store_true',
                       help='Deploy as serverless endpoint (pay-per-use, CPU only, no GPU)')
    args = parser.parse_args()
    
    print("=" * 60)
    if args.serverless:
        print("SageMaker Serverless Inference Deployment (Pay-Per-Use)")
    else:
        print("SageMaker Asynchronous Inference Deployment (Persistent)")
    print("=" * 60)
    ...
    try:
        if args.serverless:
            print("\n💰 SERVERLESS MODE: Pay only for inference time (no idle costs)")
            print("⚠️  CPU only (no GPU) - Cellpose will be slower")
            if not image_uri:
                print("\n❌ Error: --serverless requires --image-uri")
                print("Please provide Docker image URI with --image-uri")
                exit(1)
            predictor = deploy_serverless_endpoint(model_uri, image_uri=image_uri)
        else:
            print("\n💰 ASYNC MODE: Persistent GPU instance (~$0.736/hour)")
    ...
    if args.serverless:
        print(f"Mode: Serverless (CPU only)")
        print(f"Memory: 6144 MB")
        print(f"💰 Pricing: ~$0.000012/second of inference time")
        print(f"💰 $0 when idle (no constant costs!)")
        print("=" * 60)

        print("\nTo invoke the serverless endpoint:")
        print("""
    import boto3
    import json
    import base64
    ...
    # Invoke serverless endpoint (synchronous)
    ...
    """)
        ...
        print(f"💰 Cost: ~$0.736/hour (constant, even when idle)")
        print("=" * 60)

        print("\nTo invoke the async endpoint:")
        print("""
    import boto3
    import json
    import base64
